...
public class JBPMRestManagementClient {
    ...
    private DefaultHttpClient httpClient; // keep this out of the method in
					  // order to reuse the object for
					  // calling other services without
					  // losing session
    protected String address;
    protected ResourceBundle bundle;

    protected String urlForm(String prop) {
	return "http://" + bundle.getString("process.host") + bundle.getString("process.urlContext")
    ...
    protected String urlFormLocal(String prop, String... params) {
	String msg = bundle.getString(prop);
	msg = MessageFormat.format(msg, params);
	return "http://" + bundle.getString("process.host") + bundle.getString("processdetail.urlContext") + msg;
    }
	...
	try {
	    ...
	    System.out.println("::: RESPUESTA AUTENTIFICACION:::" + lol);
	} catch (Exception e) {
	    e.printStackTrace();
	}
    }
	...
	try {
	    UrlEncodedFormEntity entity = new UrlEncodedFormEntity(formparams, "UTF-8");// UrlEncodedFormEntity(formparams,
											// "multipart/form-data");
    ...
    protected String getDataFromRestService(String url, EnumJBPMRestType enumRest, Map<String, Object> parameters)
	    throws Exception {
	String json = "";

	if (EnumJBPMRestType.MULTIPART.codigo.equals(enumRest.getCodigo())) {
	...
	if (EnumJBPMRestType.POST.codigo.equals(enumRest.getCodigo())) {
	...
	if (EnumJBPMRestType.GET.codigo.equals(enumRest.getCodigo())) {
