...
public abstract class TaskServiceBaseSyncTest extends BaseTest {
    ...
    public void testPotentialOwnerQueries() {
        ...
        List<TaskSummary> actual = client.getTasksAssignedAsPotentialOwner(users.get("bobba").getId(), "es_CL");
        assertEquals(2,
                actual.size());
    }
    ...
    public void testPeopleAssignmentQueries() {
        ...
        List<TaskSummary> actual = client.getTasksAssignedAsTaskInitiator(users.get("darth").getId(), "es_CL");
        assertEquals(1,
                actual.size());
        assertTrue(CollectionUtils.equals(expected.get("darth"),
                actual));
        ...
        actual =
                client.getTasksAssignedAsBusinessAdministrator(users.get("steve").getId(), "es_CL");
        assertTrue(CollectionUtils.equals(expected.get("steve"),
                actual));
